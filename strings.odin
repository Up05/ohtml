...
index_str :: proc(haystack: string, needle: string, offset := 0) -> (int, int) {
    ...
    for _, i in haystack[offset:] {
        defer c += 1
        if starts_with(haystack[i + offset:], needle) { return i + offset, c + offset } // technically, I would need to do rune_count(offset), but whatever
    }
    return -1, -1
}
...
index_rune :: proc(haystack: string, needle: rune, offset := 0) -> (int, int) {
    ...
    for r, i in haystack[offset:] {
        defer c += 1
        if r == needle { return i + offset, c + offset }
    }
    return -1, -1
}
...
first_rune :: proc(s: string) -> rune {
    for r in s { return r } // TODO later replace with an actual impl, maybe
    panic("string is empty, cannot get first rune!")
}

any_of_any :: proc(a: $T, bees: ..T) -> bool {
    for b in bees { if a == b { return true } }
    return false
}

any_of_str :: proc(a: string, strs: ..string) -> bool {
    for b in strs { if eq(a, b) { return true } }
    return false
}
...
is_not_special :: proc(str: string) -> bool {
    for r in str {
        if is_special_rune(r) { return false }
    }
    return true
}
