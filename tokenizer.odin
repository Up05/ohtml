    ...
    for r, i in html {
        ...
        case '<':
            if is_start_of_comment(token) {
                j, c := index(token, "-->")
                if j == -1 do j, c = len(token), len(token)
                skip = c + len("<!---->")-1
                break
            }
            if is_start_of_script(token[1:]) do skip_embedded_script = true
        ...
        case '>': 
            ...
            if skip_embedded_script {
                ...
                if j > 0 do append(&tokens, token[1:j+1])
        ...
        case ' ', '\t', '\n', '\r':
            ...
            for r2, j in token {
                defer c += 1
                if is_ws_rune(r2) do continue
                skip = c - 1
                // (  optimization  )
                if j != 1 || r != ' ' do append(&tokens, token[:j])    
                break
            }
        ...
        case:
            ...
            for r2, j in token {
                defer c += 1
                if !is_ws_rune(r2) && !is_special_rune(r2) do continue
